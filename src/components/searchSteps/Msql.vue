...
import Schedule from '@/components/searchSteps/Schedule.vue'
...
q-page
	...
	.container
		.grid
			q-list.left(dense)
				q-item(clickable v-for="item in tabs.sqlSections" :key="item.id" :class="{ selected: selected == item.label }" @click="select(item)")
					q-item-section
			...
			q-scroll-area.right
				.q-ml-lg
					...
					#commonSqlProps.zg Общие настройки
					commonSqlProps(:db='props.id')
